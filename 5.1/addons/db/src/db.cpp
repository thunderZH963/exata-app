                                    ...
                                    NodeInput* nodeInput)
{
...
#ifdef MGRS_ADDON
        else if (partition->terrainData->getCoordinateSystem() == MGRS)
        {
            columns.push_back(string_pair("MGRS", "real"));
            columns.push_back(string_pair("Z", "real"));
        }
#endif // MGRS_ADDON
        else
    ...
    if (db->statsNodeStatus->isVelocity)
    {
        if (partition->terrainData->getCoordinateSystem() == CARTESIAN
#ifdef MGRS_ADDON
            || partition->terrainData->getCoordinateSystem() == MGRS
#endif // MGRS_ADDON
            )
        {
...
StatsDBSendLinkUtilizationTimerMessage(PartitionData* partition)
{
    ...
    StatsDb* db = partition->statsDb;
    clocktype currentTime = partition->getGlobalTime();
    ...
    if (db->StatsDBLinkUtilTable->createWNWLinkUtilizationTable)
    {
        ...
        LinkUtilizationPerNodeTimer->eventTime = partition->getGlobalTime() +
            db->StatsDBLinkUtilTable->wnwLinkUtilizationTableInterval;
        ...
        {// for linktuil table and linkutil per frame tanble.
            ...
            LinkUtilizationTimer->eventTime = partition->getGlobalTime() +
                db->StatsDBLinkUtilTable->wnwLinkUtilizationTableInterval;
...
void STATSDB_ProcessEvent(PartitionData *partition, Message *msg)
{
    ...
    switch (msg->eventType)
    {
...
#ifdef ADDON_BOEINGFCS
        case MSG_STATS_SRW_IslandStatusTable:
        {

            StatsDBSrwTableBase *tablePtr = db->statsSrwTableContainer->getTableBasePtr(DB_SRW_ISLAND_STATUS);
            ERROR_Assert(tablePtr != NULL, "SRW Island Status Table is NULL");

            // Process the Event.
            tablePtr->processEvent(node, msg);

            // Renew the update timer.
            MESSAGE_Send(node,
                         tablePtr->getUpdateTimerMessage(),
                         tablePtr->getUpdateInterval());

            break;
        }
        case MSG_STATS_SRW_IntranetRoutingStatusTable:
        {

            StatsDBSrwTableBase *tablePtr = db->statsSrwTableContainer->getTableBasePtr(DB_SRW_INTRANET_ROUTING_STATUS);
            ERROR_Assert(tablePtr != NULL, "SRW Intranet Routing Table is NULL");
        ...
        case MSG_STATS_SRW_AdjacencyStatusTable:
        {

            StatsDBSrwTableBase *tablePtr = db->statsSrwTableContainer->getTableBasePtr(DB_SRW_ADJACENCY_STATUS);
            ERROR_Assert(tablePtr != NULL, "SRW Adjacency Status Table is NULL");
