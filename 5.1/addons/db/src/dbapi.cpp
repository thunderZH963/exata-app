                                     ...
                                     BOOL triggered)
{
    ...
    {
        m_DimensionOnePosition = node->mobilityData->current->position.common.c1;
        m_DimensionTwoPosition = node->mobilityData->current->position.common.c2;
        m_DimensionThreePosition = node->mobilityData->current->position.common.c3;
    }
#endif // MGRS_ADDON
    ...
    if (wasFound && strcmp(buf, "YES") == 0)
    {
        ...
        {
            m_DimensionOneVelocity = node->mobilityData->lastExternalVelocity.common.c1;
            m_DimensionTwoVelocity = node->mobilityData->lastExternalVelocity.common.c2;
            m_DimensionThreeVelocity = node->mobilityData->lastExternalVelocity.common.c3;
        }
#endif // MGRS_ADDON
    }
    else
...
void STATSDB_HandleNodeStatusTableInsert(Node* node, StatsDBNodeStatus nodeStatus)
{
    ...
    if (db->statsNodeStatus->isPosition)
    {
        ...
        {            
            columns.push_back("MGRS");
            columns.push_back("Z");
            if (nodeStatus.m_PositionUpdated)
            {              
                MgrsUtility mgrs;
                std::string sw = partition->terrainData->getMgrsSw();

                sw = mgrs.posToMgrs(sw, nodeStatus.m_DimensionOnePosition,
                    nodeStatus.m_DimensionTwoPosition);

                newValues.push_back(sw);
                newValues.push_back(
                    STATSDB_DoubleToString(nodeStatus.m_DimensionThreePosition));
            }
            else
        ...
        const char *eventType)
{
    ...
    if (db->statsNetEvents->multipleValues)
    {
        if (db->engineType == UTIL::Database::dbMySQL)
        {
            ...
            Int32 requiredBytes = (Int32)strlen(buf);
                ...
                ipEvent->bufferSizeInBytes - db->networkEventsBytesUsed)
            {
                ...
                // for now
                requiredBytes += (Int32)strlen(ipEvent->networkEventsTbColsName) - 1;
            }
               ...
               iterTmp = linkUtilBufferPerNode->begin();
                while (iterTmp != linkUtilBufferPerNode->end())
                {
                    ...
                    while (iterSlotTmp != slotDespTmp->end())
                    {
                        ...
                        columns.push_back("Timestamp");
                        newValues.push_back(
                            STATSDB_DoubleToString((double) traverseNode->getNodeTime() / SECOND));
                        columns.push_back("NodeId");
