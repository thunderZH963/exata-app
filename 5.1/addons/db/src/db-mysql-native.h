...
struct MysqlNativeDriver : public DatabaseDriver
{
    ...
    void setup() {
        ...
        std::string fullpath;        
        fullpath = productHomePath;
        
#ifdef WINDOWS_OS
        HINSTANCE dllHandle = NULL;
#ifdef _WIN64
        fullpath += "/lib/windows-x64/libmysql.dll";
#else
        fullpath += "/lib/windows/libmysql.dll";
#endif
...
#ifdef _LP64
        fullpath += "/lib/linux-common-x64/libmysql.so";
#else
        fullpath += "/lib/linux-common/libmysql.so";
#endif
        //Load the dll and keep the handle to it
    ...
    void open (bool dropDatabase)
    {
        ...
        {
            connect = mysql_real_connect_ptr(&conn,
                                             server.c_str(),
                                             username.c_str(),
                                             password.c_str(),
                                             NULL,
        ...
        else
        {
            connect = mysql_real_connect_ptr(&conn,
                                             ...
                                             NULL,
                                             0,
                                             socketFilePath.c_str(),
                                             0);
        }
        if (!connect)
        {
            sprintf(errBuf, "Cannot connect to MYSQL server: %s.\n",
                    error());
            if (errorNum() == 2002) // For CR_CONNECTION_ERROR
            {
                sprintf(errBuf + strlen(errBuf), "Please specify the "
                    "correct socket file path (absolute path with file "
                    "name) using the parameter "
                    "STATS-DB-MYSQL-SOCKET-FILE-PATH in .config file.");
            }
