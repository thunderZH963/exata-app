...
// begin: SOPS state
static inline
void SetOculaPropertyExataNetLink(Node* node, int previousHopId, int bytesRcvd)
{
    // persistent state hack
    const int MAX_NUM_NODES = 4000;
    static unsigned packetsReceived[MAX_NUM_NODES];
    static unsigned bytesReceived[MAX_NUM_NODES];
    ...
    if (OculaInterfaceEnabled())
    {
        ...
        ++packetsReceived[node->nodeId];
        bytesReceived[node->nodeId] += bytesRcvd;

        sprintf(key, "/node/%d/exata/netLink/%d/previousHopId", node->nodeId, previousHopId);
        ...
        sprintf(key, "/node/%d/exata/netLink/%d/packetsReceived", node->nodeId, previousHopId);
        ...
        sprintf(key, "/node/%d/exata/netLink/%d/bytesReceived", node->nodeId, previousHopId);
...
void SetOculaPropertyExataAppLink(Node* node, int sourceId, int bytesRcvd)
{
    // persistent state hack
    const int MAX_NUM_NODES = 4000;
    static int packetsReceived[MAX_NUM_NODES];
    static unsigned bytesReceived[MAX_NUM_NODES];
    ...
    if (OculaInterfaceEnabled())
    {
        ...
        sprintf(key, "/node/%d/exata/appLink/%d/sourceId", node->nodeId, sourceId);
        ...
        sprintf(key, "/node/%d/exata/appLink/%d/packetsReceived", node->nodeId, sourceId);
        sprintf(value, "%u", packetsReceived[node->nodeId]);
        SetOculaProperty(
            node->partitionData,
            key,
        ...
        sprintf(key, "/node/%d/exata/appLink/%d/bytesReceived", node->nodeId, sourceId);
        sprintf(value, "%u", bytesReceived[node->nodeId]);
        SetOculaProperty(
            node->partitionData,
            key,
...
/**!
 \todo On checkin to 4.2.1 -- is this OK to have this here or should I encapsulate?
 Right now we only support SRW so the issue is I use a SRW-specific define (first of all)
 and second this code is in the middle of network_ip.cpp, which isn't a very obvious
 place for it.  
 */
        if (!useCellular) /// Only SRW support right now
        {
            MacData* macData = (MacData*)node->macData[outgoingInterface];
            if (macData->macProtocol != MAC_PROTOCOL_CES_SRW_PORT) 
            {
#ifdef DEBUG
              printf("--> Route is not SRW...clearing.\n");
#endif
              useCellular = true;
              routeIndicator = 'M';
            }
        ...
        if (inComingData)
        {
                ...
                !NetworkIpIsMulticastAddress(node, ipHeader->ip_dst))
            {
                    ...
                    ipHeader->ip_p == IPPROTO_TCP)
                {
                    ...
                    {
                        ip->interfaceInfo[interfaceIndex]
                        ->firstInUcastDataPacketTime = node->getNodeTime();
                    }
                    ip->interfaceInfo[interfaceIndex]
                    ->lastInUcastDataPacketTime = node->getNodeTime();
                }
            }
                        ...
                        NodeAddress previousHopAddress)
{
    ...
    if (node->networkData.networkProtocol == CELLULAR)
    {
             ...
             CellularLayer3IsPacketForMyPlmn(node, msg, incomingInterface, NETWORK_IPV4))
        {
            ...
            {
                MacData* macData = (MacData*)node->macData[outgoingInterface];
                if (macData->macProtocol != MAC_PROTOCOL_CES_SRW_PORT) //XXX
                {
                  useCellular = true;
                  routeIndicator = 'M';
                }
...
#ifdef MUOS_LIB
        if (CellularLayer3IsUserDevices(node, incomingInterface))
        {
            NetworkDataIp *ip = (NetworkDataIp*) node->networkData.networkVar;
            NetworkDataIcmp* icmp = (NetworkDataIcmp*) ip->icmpStruct;
            IpHeaderType* ipHeader = (IpHeaderType*)msg->packet;
...
#if defined(DEBUG) && DEBUG > 1
            printf("Node[%d]: RoutePacketAndSendToMac(Path 2) UE routing packet from 0x%x to 0x%x\n", 
                   node->nodeId, ipHeader->ip_src, ipHeader->ip_dst);
            ...
            if (useCellular)
            {
              ...
              return;
              //
              // This is a placeholder.  Do not remove it.
              //
              // Maintainer's node (Jeff Weaver)
              // This code is not presently used. An earlier developer created IP-IP encapsulation.
              // I didn't use it in the beta release but that doesn't mean this code doesn't have value
              // in a future release.  I don't know if it goes here or not because I intend (when I get
              // the chance) for a future developer to rework this section of network_ip.cpp to make it
              // more maintainable.  
              //
              // So this code was a stub to deliver a packet into Li's IP-IP encapsulation code.  I've
              // changed the code around the entry point substantially.  If you uncomment it, it might
              // work, it probably won't.  Her code re-enters network_ip.cpp with a new IPv4 header.
              //
              //if (Muoslayer3HandlePacketFromIpHost(node, msg, ipHeader, incomingInterface)) { return; }
              //
            }
            else
            {
