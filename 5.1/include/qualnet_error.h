...
#define ERROR_AssertArgs(expr, ...) \
    {\
        ...
        {\
            Int32 txtLen;\
            txtLen = snprintf(NULL, 0, __VA_ARGS__);\
            txtLen++;\
            char* txt = new char[txtLen];\
            snprintf(txt, txtLen, __VA_ARGS__);\
            (void)ERROR_WriteError(ERROR_ASSERTION,\
                                   #expr,\
                                   txt,\
                                   __FILE__,\
                                   __LINE__);\
...
/*
 * FUNCTION     ERROR_ReportErrorArgs
 * PURPOSE      Function call used to report an error condition with
 *              variable arguments. This function in turns calls 
 *              ERROR_ReportError.
 *
 * Parameters:
 *    warning:  the error message string
 */
...
/*
 * FUNCTION     ERROR_ReportWarningArgs
 * PURPOSE      Function call used to report a recoverable error condition
 *              with variable arguments. This function in turns calls 
 *              ERROR_ReportWarning.
 *
 * Parameters:
