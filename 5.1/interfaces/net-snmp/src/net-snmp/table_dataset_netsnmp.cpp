...
void
NetSnmpAgent::_table_set_add_indexes(netsnmp_table_data_set* table_set, struct tree* tp)
{
    ...
    for (index = tp->indexes; index; index = index->next) {
        if (!snmp_parse_oid(index->ilabel, name, &name_length) ||
            (NULL ==
             (indexnode = get_tree(name, name_length, tree_head)))) {
            return;             /* xxx mem leak */
        }
...
void
NetSnmpAgent::netsnmp_config_parse_table_set(const char* token, char* line)
{
    ...
    if (NULL != tp->augments) {
        ...
        if (NULL == (tp2 = get_tree(name, name_length, tree_head))) {
            SNMP_FREE (table_set);
            return;
        }
...
void
NetSnmpAgent::netsnmp_config_parse_add_row(const char* token, char* line)
{
...
void
NetSnmpAgent::netsnmp_init_table_dataset(void) {
#ifndef NETSNMP_DISABLE_MIB_LOADING
    register_app_config_handler("table",
                                &NetSnmpAgent::netsnmp_config_parse_table_set, NULL,
                                "tableoid");
#endif /* NETSNMP_DISABLE_MIB_LOADING */
    register_app_config_handler("add_row",
                                &NetSnmpAgent::netsnmp_config_parse_add_row,
...
*/
int
NetSnmpAgent::netsnmp_register_table_data_set(netsnmp_handler_registration* reginfo,
...
/** registers a table_dataset so that the "add_row" snmpd.conf token
  ...
  */
void
NetSnmpAgent::netsnmp_register_auto_data_table(netsnmp_table_data_set* table_set,
...
netsnmp_mib_handler*
NetSnmpAgent::netsnmp_get_table_data_set_handler(netsnmp_table_data_set* data_set)
{
    ...
    (netsnmp_table_data_set_storage* defrow)
{
    ...
    for (; defrow; defrow = defrow->next) {
        netsnmp_set_row_column(row, defrow->column, defrow->type,
                               (const char*)defrow->data.voidp, defrow->data_len);
        if (defrow->writable)
...
int
NetSnmpAgent::netsnmp_table_data_set_helper_handler(netsnmp_mib_handler* handler,
                                      ...
                                      Node* node = NULL)
{
    ...
    for (request = requests; request; request = request->next) {
        ...
        const oid*  const suffix = (const oid*)
        ...
        if (MODE_IS_SET(reqinfo->mode)) {
            ...
            if ((-1 == rc) || ((unsigned int)rc >= sizeof(buf))) {
            ...
            stashp = (netsnmp_oid_stash_node**)
                netsnmp_table_get_or_create_row_stash(reqinfo, (const u_char*)buf);
        ...
        case MODE_SET_RESERVE2:
            ...
            data->data.string =
                (u_char*)netsnmp_strdup_and_null(request->requestvb->val.string,
                                        (size_t)request->requestvb->val_len);
            if (!data->data.string) {
