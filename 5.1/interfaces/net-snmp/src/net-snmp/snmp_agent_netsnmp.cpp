...
void
NetSnmpAgent::free_agent_snmp_session(netsnmp_agent_session* asp)
{
    if (!asp)
        return;
...
int NetSnmpAgent::agent_check_and_process(int block)
{
#ifdef SNMP_DEBUG
 printf("agent_check_and_process:node %d\n",node->nodeId);
#endif

    int             count = 1;
    int             fakeblock = 0;

    snmp_select_info(&fakeblock);
...
int
NetSnmpAgent::netsnmp_remove_from_delegated(netsnmp_agent_session* asp)
{
...
void
NetSnmpAgent::netsnmp_remove_and_free_agent_snmp_session(netsnmp_agent_session* asp)
{
...
void
NetSnmpAgent::netsnmp_free_agent_request_info(netsnmp_agent_request_info* ari)
{
    if (ari) {
        if (ari->agent_data) {
                            ...
                            const char* name)
{
 ...
 void
     NetSnmpAgent::netsnmp_free_agent_data_sets(netsnmp_agent_request_info* ari)
{
    if (ari) {
        netsnmp_free_all_list_data(ari->agent_data);
    }
...
u_long
netsnmp_get_agent_uptime(Node* node)
{
    struct timeval  now;
    now = convert_2_timeval(node->getNodeTime());
...
int
NetSnmpAgent::netsnmp_agent_check_packet(netsnmp_session*  session,
                           ...
                           void* transport_data, int transport_data_length)
{
    ...
    if (transport != NULL && transport->f_fmtaddr != NULL) {
         ...
         */
        addr_string = (this->*(transport->f_fmtaddr))(transport, transport_data,
...
int
NetSnmpAgent::netsnmp_agent_check_parse(netsnmp_session*  session,
                                        netsnmp_pdu* pdu, int result)
{
    if (result == 0) {
        return 1;
    }
 ...
 */
int
NetSnmpAgent::netsnmp_addrcache_add(const char* addr)
{
...
int
NetSnmpAgent::netsnmp_check_delegated_chain_for(netsnmp_agent_session* asp)
{
...
int
NetSnmpAgent::netsnmp_add_queued(netsnmp_agent_session* asp)
{
...
int
NetSnmpAgent::get_set_cache(netsnmp_agent_session* asp)
{
...
int
NetSnmpAgent::netsnmp_create_subtree_cache(netsnmp_agent_session* asp)
{
    netsnmp_subtree* tp;
    ...
    if (asp->treecache == NULL && asp->treecache_len == 0) {
        ...
        asp->treecache = (netsnmp_tree_cache*)
    ...
    for (varbind_ptr = asp->pdu->variables; varbind_ptr;
        ...
        if (view == VACM_SUCCESS) {
            request = netsnmp_add_varbind_to_cache(asp, vbcount,
                                                   varbind_ptr, tp);
 ...
 */
int
NetSnmpAgent::handle_pdu(netsnmp_agent_session* asp)
{
#ifdef SNMP_DEBUG
    printf("handle_pdu:node %d\n",node->nodeId);
#endif
    ...
    case SNMP_MSG_GETBULK:     /* note: there is no getbulk stat */
        if (asp->mode == SNMP_MSG_GETBULK){
            snmp_increment_statistic(STAT_INBULKREQ);
        }
        /*
...
int
NetSnmpAgent::netsnmp_check_for_delegated_and_add(netsnmp_agent_session* asp)
{
...
int
NetSnmpAgent::netsnmp_wrap_up_request(netsnmp_agent_session* asp, int status)
{
#ifdef SNMP_DEBUG
    printf("netsnmp_wrap_up_request:node %d\n",node->nodeId);
#endif
...
int
NetSnmpAgent::netsnmp_handle_request(netsnmp_agent_session* asp, int status)
{
#ifdef SNMP_DEBUG
    printf("netsnmp_handle_request:node %d\n",node->nodeId);
#endif
...
NetSnmpAgent::netsnmp_add_varbind_to_cache(netsnmp_agent_session* asp, int vbcount,
...
#define CACHE_GROW_SIZE 16
        ...
        if (asp->pdu->command == SNMP_MSG_GETNEXT ||
            asp->pdu->command == SNMP_MSG_GETBULK) {
            request->range_end = (u_long*)tp->end_a;
 ...
 */
int
NetSnmpAgent::check_acm(netsnmp_agent_session* asp, u_char type)
{
    int             view;
    int             i, j, k;
    netsnmp_request_info* request;
...
int
NetSnmpAgent::handle_var_requests(netsnmp_agent_session* asp)
{
#ifdef SNMP_DEBUG
    printf("handle_var_requests:node %d\n",node->nodeId);
#endif
 ...
 */
int
NetSnmpAgent::netsnmp_reassign_requests(netsnmp_agent_session* asp)
{
    /*
...
int
NetSnmpAgent::handle_getnext_loop(netsnmp_agent_session* asp)
{
    int             status;
...
int
NetSnmpAgent::handle_set(netsnmp_agent_session* asp)
{
    int             status;
    /*
...
int
NetSnmpAgent::handle_set_loop(netsnmp_agent_session* asp)
{
...
int
NetSnmpAgent::netsnmp_check_for_delegated(netsnmp_agent_session* asp)
{
    int             i;
    netsnmp_request_info* request;
...
agent_set_cache*
NetSnmpAgent::save_set_cache(netsnmp_agent_session* asp)
{
    agent_set_cache* ptr;
