...
void AdjustSubnets::show()
{
    ...
    tableWidget->horizontalHeaderItem(0)->setToolTip(QString("This is the name of the radio. It consists of the ") + extractor->productName + QString(" name and the interface id"));
    ...
    QStringList addrs;
    SNT_SYNCH::NetworkSet& nets = extractor->ns->networksUsed();
    SNT_SYNCH::NetworkSet::iterator netIt = nets.begin();
    ...
    while (it != extractor->ns->end())
    {
        ...
        if ((*it)->entity)
        {
            SNT_SYNCH::RadioStruct* rit = (*it)->radio;
            ...
            QString name = QString((*it)->getNodeName()) + ":" +
                                QString::number(rit->radioIndex);
            QString addr(extractor->ns->
                getNetworkAddress(*it, rit).c_str());
            ...
            tableWidget->item(row,0)->setToolTip(QString("This is the name of the radio. It consists of the ") + extractor->productName + QString(" name and the interface id"));
...
void AdjustSubnets::accept()
{
    for (int row=0; row<tableWidget->rowCount(); row++)
    {
        ...
        {
            SNT_SYNCH::Network* net = extractor->ns->getNetwork(
                                        box->currentText().toAscii().data());
            if (net)
            {
