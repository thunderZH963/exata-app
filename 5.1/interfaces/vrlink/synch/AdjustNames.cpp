...
void AdjustNames::show()
{
    ...
    tableWidget->horizontalHeaderItem(0)->setToolTip(QString("This is the node id used by the ") + extractor->productName + QString(" simulator and GUI"));
    tableWidget->horizontalHeaderItem(1)->setToolTip("This is the name used by the HLA Federate/DIS Exercise that published the entity");
    tableWidget->horizontalHeaderItem(2)->setToolTip(QString("Enter text to change the name that will be used by the ") + extractor->productName + QString(" simulator and GUI"));
    tableWidget->horizontalHeaderItem(2)->setText(extractor->productName + QString(" Name"));
    tableWidget->horizontalHeaderItem(3)->setToolTip(QString("Enter an icon file name or browse to change the icon used by the ") + extractor->productName + QString(" simulator and GUI"));

    QString htmlToolTip = tableWidget->toolTip();
    htmlToolTip.replace(QString("EXata/QualNet"), extractor->productName);
    tableWidget->setToolTip(htmlToolTip);

    QAbstractItemModel* model = tableWidget->model();
    SNT_SYNCH::NodeSet::iterator it = extractor->ns->begin();
    ...
    while (it != extractor->ns->end())
    {
        ...
        if ((*it)->entity)
        {
            ...
            wid->setToolTip(QString("Enter an icon file name or browse to change the icon used by the ") + extractor->productName + QString(" simulator and GUI"));
            tableWidget->setCellWidget(row, 3, wid);
            tableWidget->item(row,0)->setToolTip(QString("This is the node id used by the ") + extractor->productName + QString("simulator and GUI"));
            tableWidget->item(row,1)->setToolTip("This is the name used by the HLA Federate/DIS Exercise that published the entity");
            tableWidget->item(row,2)->setToolTip(QString("Enter text to change the name that will be used by the ") + extractor->productName + QString(" simulator and GUI"));
...
void AdjustNames::accept()
{
    ...
    for (int row=0; row<tableWidget->rowCount(); row++)
    {
        ...
        SNT_SYNCH::Node* node = extractor->ns->findByNodeID(id);
        if (node && node->entity)
        {
            index = model->index(row, 2, QModelIndex());
