...
// PURPOSE    :: get number of lines in the file
// PARAMETERS ::
// + path : path to the file
// RETURN     ::  unsigned.
// **/
static unsigned GetNumLinesInFile(const char* path)
{
    FILE* fp = fopen(path, "r");
    if (fp == NULL)
    {
        cerr << "Can't open for reading : " <<  path;
        exit(EXIT_FAILURE);
    }
    ...
    while (1)
    {
        ...
        if (strlen(line) >= lineBufSize)
        {
            cerr << "Exceeds permitted line length" << path;
            exit(EXIT_FAILURE);
        }
    }
