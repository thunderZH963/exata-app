...
// RETURN     ::  Void : NULL
// **/
void MTS_PrintMessageList(const char* prefix, std::vector<MTS_MessageList*>& list)
{
    ...
    char *fileName)
{
    ...
    while (!feof(f))
    {
        ...
        else if (strcmp(strupcase(token), "WARMUP") == 0)
        {
            state->doWarmup = TRUE;
        }
