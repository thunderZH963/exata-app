...
{ 
    ...
    else if (requestMessage->m_Protocol == SocketInterface_ProtocolType_ProtocolTCP)
    {
        ...
        else
        {
            if (requestMessage->m_FailureTimeoutSpecified)
            {
                record->time = node->getNodeTime() + (clocktype) (requestMessage->m_FailureTimeout * SECOND);
            ...
            {
                record->time = node->getNodeTime() + data->tcpFailureTimeout;
                if (data->deterministicTiming)
                {
        ...
        if (requestMessage->m_TtlSpecified)
        {
            ...
            EXTERNAL_SendVirtualDataAppLayerTCP(
                ...
                requestMessage->m_Size, // All MTS data are virtual
    ...
    else if (requestMessage->m_Protocol == SocketInterface_ProtocolType_ProtocolUDP)
    {
        ...
        else
        {
            ...
            {
                record->time = node->getNodeTime() + data->udpFailureTimeout;
                if (data->deterministicTiming)
                {
     ...
     double coord3)
{
    ...
    // Removed: check is not valid because max altitude is not specified
#if 0
    if (coord3 < origin.z)
    {
        sprintf(
    ...
    std::vector<std::string>* leaveGroups)
{
    ...
    clocktype delay;
    if (node->getNodeTime() > eventTime)
    {
        delay = 0;
    }
    else
    {
        delay = eventTime - node->getNodeTime();
    }
    ...
    clocktype delay)
{
    ...
    {
        sprintf(errString,
            "ERROR: O:%" TYPES_64BITFMT "u C:%" TYPES_64BITFMT "u",
            message->m_Id1,
            message->m_Id2);
    ...
    SimulationProperties* simProps)
{
    ...
    strcpy(defCesPath, simProps->qualnetHomePath.c_str());
    //strcpy (simProps->qualnetHomePath, "C:/navin");

    const char* found = strchr(simProps->qualnetHomePath.c_str(), '/');
    if (found == NULL)
    {
    ...
    HandleConfigurationFile((char*) simProps->configFileName.c_str(),
                            ...
                            "ces.parser.rc");
    simProps->configFileName = cesConfigurationFile;
#endif
}
    ...
    PartitionData * partitionData)
{
    ...
    if (serverMode)
    {
        ...
        fileString[MAX_STRING_LENGTH - 1] = '\0';
        simProps->configFileName = fileString;        
    }
