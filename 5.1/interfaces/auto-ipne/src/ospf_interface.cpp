...
void OSPFPacketHandler::ReformatIncomingPacket(unsigned char *packet)
{
    struct libnet_ipv4_hdr* ip;
    UInt8* next;

    // Find the ospf header
    next = packet;// + AutoIPNE_ETHERNET_HEADER_LENGTH;
    ...
    // Necessary, not sure why
     ...
     if (ospfCommon->packetType==0x01)
     {
         if (ip->ip_dst.s_addr == 0x050000e0)
        //if (ip->ip_dst.s_addr == htonl(0x050000e0))
        {
...
void OSPFPacketHandler::ReformatOutgoingPacket(unsigned char *packet)
{
    OspfSwapBytes(packet, FALSE);
}
...
// **/
UInt16 OSPFPacketHandler::ComputeOspfChecksum(
    UInt8* packet,
    int packetLength)
{
    BOOL odd;
...
// **/
void OSPFPacketHandler::OspfSwapHello(Ospfv2CommonHeader* ospfCommon)
{
...
void OSPFPacketHandler::OspfSwapRospfDbdg(
    Ospfv2CommonHeader* ospfCommon, 
    BOOL in)
{
    RoutingCesRospfDbdgPacketHeader* dbdgPacket = 
        reinterpret_cast<RoutingCesRospfDbdgPacketHeader*>(ospfCommon);

    EXTERNAL_ntoh(&dbdgPacket->compartmentsRemaining, 
                    sizeof(dbdgPacket->compartmentsRemaining));
    EXTERNAL_ntoh(&dbdgPacket->numDigests, 
                    sizeof(dbdgPacket->numDigests));
    EXTERNAL_ntoh(&dbdgPacket->numAcks, 
                    sizeof(dbdgPacket->numAcks));
    EXTERNAL_ntoh(&dbdgPacket->interfaceMtu, 
                    sizeof(dbdgPacket->interfaceMtu));
    EXTERNAL_ntoh(&dbdgPacket->sessionID, 
                    sizeof(dbdgPacket->sessionID));

    char* msgPtr = ((char*) dbdgPacket) + sizeof(RoutingCesRospfDbdgPacketHeader);

    for (int i = 0; i < dbdgPacket->numAcks; i++)
    {
        RoutingCesRospfDbdgAck* ackPtr = 
            reinterpret_cast<RoutingCesRospfDbdgAck*>(msgPtr);;

        EXTERNAL_ntoh(&ackPtr->compartmentID[0],
            sizeof(ackPtr->compartmentID[0]));
        EXTERNAL_ntoh(&ackPtr->compartmentID[1],
            sizeof(ackPtr->compartmentID[1]));
        msgPtr += ackPtr->size();
    }
    for (int i = 0; i < dbdgPacket->numDigests; i++)
    {
        RoutingCesRospfDbdgDigest* digestPtr =
            reinterpret_cast<RoutingCesRospfDbdgDigest*>(msgPtr);
        EXTERNAL_ntoh(&digestPtr->compartmentID[0],
            sizeof(digestPtr->compartmentID[0]));
        EXTERNAL_ntoh(&digestPtr->compartmentID[1],
            sizeof(digestPtr->compartmentID[1]));
        msgPtr += digestPtr->size();
    }
}
#endif
...
// + in : BOOL : TRUE if the packet is coming in to QualNet, FALSE if outgoing
// **/
void OSPFPacketHandler::OspfSwapDatabaseDescription(
    Ospfv2CommonHeader* ospfCommon,
    BOOL in)
{
    ...
    EXTERNAL_swapBitfield(&db->ospfDbDp, 1);
...
// + ospfCommon : Ospfv2CommonHeader* : The start of the common header
// **/
void OSPFPacketHandler::OspfSwapLinkStateRequestBytes(Ospfv2CommonHeader* ospfCommon)
{
...
// **/
Ospfv2LinkStateHeader* OSPFPacketHandler::OspfSwapLinkStateBytes(
    ...
    BOOL computeChecksum)
{
    int i;
    int j;
    Ospfv2LinkStateHeader* newLsh = NULL;
    short int lsLength;
    ...
    default:
    {
        ...
        if (in)
        {
            ERROR_ReportWarning("[IPNE interface] Unknown Incoming LSA type");
        }
        else
        {
            ERROR_ReportWarning("[IPNE interface] Unknown outgoing LSA type");
        }
    }
        newLsh = (Ospfv2LinkStateHeader*)
            ( ((char*) lsh) + lsLength );
...
// + in : BOOL : TRUE if the packet is coming in to QualNet, FALSE if outgoing
// **/
void OSPFPacketHandler::OspfSwapLinkStateUpdateBytes(
    Ospfv2CommonHeader* ospfCommon,
    BOOL in)
{
...
// + in : BOOL : TRUE if the packet is coming in to QualNet, FALSE if outgoing
// **/
void OSPFPacketHandler::OspfSwapLinkStateAckBytes(
    Ospfv2CommonHeader* ospfCommon,
    BOOL in)
{
...
void OSPFPacketHandler::InitializeOspfInterface(EXTERNAL_Interface* iface)
{
    AutoIPNEData* data;
    Node* node1;
    int i;
...
void OSPFPacketHandler::OspfSwapBytes(
    UInt8* ospfHeader,
    BOOL in)
{
    Ospfv2CommonHeader* ospfCommon;
    ...
    switch (ospfCommon->packetType)
    {
        ...
        case OSPFv2_ROSPF_DBDG:
            OspfSwapRospfDbdg(ospfCommon, in);
            break;
        case OSPFv2_ROSPF_REDIRECT:
            break;
        case OSPFv2_ROSPF_CONFIG:
            break;
        case OSPFv2_ROSPF_CONFIG_ACK:
            break;
#endif
...
UInt16 OSPFPacketHandler::CalculateLinkStateChecksum(
    Node* node,
    Ospfv2LinkStateHeader* lsh)
{
    ...
    if (firstTime)
    {
        ...
        else
        {
            ...
            firstTime = FALSE;
            computeChecksum = ipneData->ospfEnabled;
        }
...
void OSPFPacketHandler::PreProcessIpPacketOspf(
...
void OSPFPacketHandler::ProcessOutgoingIpPacketOspf(
    EXTERNAL_Interface* iface,
...
void OSPFPacketHandler::IgmpSwapBytes(UInt8* igmpHeader, BOOL in)
{
    // IGMP is not supported
}

void OSPFPacketHandler::HandleIgmpInject(
    EXTERNAL_Interface* iface,
    UInt8* packet,
    int size)
{
