...
// RETURN    :: void
// **/
void AutoIPNEHandleSniffedTunnelPacket(UInt8*    packet)
{
...
// /**
// API       :: AutoIPNEHandleSniffedIPsecPacket
// PURPOSE   :: This function will handle an incoming IPsec packet received
...
// RETURN    :: void
// **/
void AutoIPNEHandleSniffedIPsecPacket(Node*         node,
                                      Message*      msg,
                                      char*         dat,
                                      IpHeaderType* ipHdr,
                                      unsigned int  proto)
{
    UInt8* nextHeader;
    int hLen = (int) IpHeaderSize(ipHdr);
    nextHeader = ((UInt8*) dat) + hLen;
    int payloadSize = (int) IpHeaderGetIpLength(ipHdr->ip_v_hl_tos_len) - hLen;
    ...
    {
        AutoIPNEHandleSniffedTunnelPacket((UInt8*)dat);

        // Add IP trace as it would have been lost
        // if received from external entity
        if (msg->isEmulationPacket)
        {
            proto = ipHdr->ip_p;
            msg->headerProtocols[msg->numberOfHeaders] = TRACE_IP;
            msg->headerSizes[msg->numberOfHeaders] = (int) IpHeaderSize(ipHdr);
            msg->numberOfHeaders++;
        }
    }
    else
    ...
    if (msg->isEmulationPacket)
    {
        ...
        {
            MESSAGE_RemoveHeader(node, msg, (int) IpHeaderSize(ipHdr), TRACE_IP);
            msg->headerProtocols[msg->numberOfHeaders] = TRACE_UDP;
            msg->headerSizes[msg->numberOfHeaders] = sizeof(TransportUdpHeader);
            msg->numberOfHeaders++;
            MESSAGE_AddHeader(node, msg, (int) IpHeaderSize(ipHdr), TRACE_IP);
        }
