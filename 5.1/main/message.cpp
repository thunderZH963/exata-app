             ...
             TraceProtocolType protocolType)
{
    ...
    // added for muos begin
    unsigned short extra_delay = 0;
    for (int i = 0; i < numFrags; ++i)
    {
        unsigned short* um_seq = (unsigned short*)
         MESSAGE_ReturnInfo(fragList[i], INFO_TYPE_MUOS_Um_Seq);

        if (um_seq && *um_seq > extra_delay)
        {
            extra_delay = *um_seq;
        }
    ...
    {
        unsigned short*  um_seq = (unsigned short*)
            MESSAGE_ReturnInfo(fragList[0], INFO_TYPE_MUOS_Um_Seq);
        if (um_seq == NULL)
        {
            MESSAGE_AddInfo(node, fragList[0], sizeof(unsigned short),
